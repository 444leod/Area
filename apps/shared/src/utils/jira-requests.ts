...
function generateDescriptionByString(description: string): {
  type: string;
  version: number;
...
export class JiraAPI {
  ...
  async getJiraDomains(): Promise<JiraSite[]> {
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        throw new Error(
          `Something went wrong while retrieving user's Atlassian domains: ${error.response?.data?.errorMessages.join(", ")}`,
        );
      } else {
        throw new Error(
          `Something went wrong while retrieving user's Atlassian domains: ${error.message}`,
        );
      }
    }
  }
    ...
    for (const domain of this.domains) {
      ...
      if (domainProjects) {
        projects = projects.concat(
          domainProjects.map((p) => ({ ...p, domain: domain })),
        );
      }
    }
  ...
  async getProjectsByDomain(domainId: string): Promise<JiraShortProject[]> {
    try {
      ...
      if (axios.isAxiosError(error)) {
        throw new Error(
          `Something went wrong while retrieving projects for domain ${domainId}: ${error.response?.data?.errorMessages.join(", ")}`,
        );
      } else {
        throw new Error(
          `Something went wrong while retrieving projects for domain ${domainId}: ${error.message}`,
        );
      }
    }
  }
   ...
   * @returns An array of JiraTicket objects
   */
  async getDomainTickets(
    domainId: string,
    params: { maxResults?: number; fields?: string; jql?: string },
  ): Promise<JiraTicket[]> {
    let tickets: JiraTicket[] = [];
    let total = 0;
    try {
      do {
        ...
        (!params.maxResults && tickets.length < total) ||
        (params.maxResults &&
          tickets.length < params.maxResults &&
          tickets.length < total)
      );
    } catch (error: any) {
      if (axios.isAxiosError(error)) {
        throw new Error(
          `Something went wrong while retrieving tickets for domain ${domainId}: ${error.response?.data?.errorMessages.join(", ")}`,
        );
      } else {
        throw new Error(
          `Something went wrong while retrieving tickets for domain ${domainId}: ${error.message}`,
        );
      }
    }
    return tickets;
  ...
  async createTicket(
    ...
    try {
      const response = await axios.post<JiraTicket>(
        this.buildJiraUrl(project.domain.id, "3/issue"),
        {
          ...
          fields: {
            ...
            description: generateDescriptionByString(
      ...
      if (axios.isAxiosError(error)) {
        throw new Error(
          `Something went wrong while creating a ticket in project ${project.key}: ${error.response?.data?.errorMessages.join(", ")}`,
        );
      } else {
        throw new Error(
          `Something went wrong while creating a ticket in project ${project.key}: ${error.message}`,
        );
      }
    }
  }
  ...
  async getProjectByKey(projectKey: string): Promise<JiraProject> {
    const projects =
      this.projects.length != 0 ? this.projects : await this.getProjects();
  ...
  async getFullProject(domain: JiraSite, id: string): Promise<JiraProject> {
    try {
      ...
      if (axios.isAxiosError(error)) {
        throw new Error(
          `Something went wrong while retrieving project ${id} in domain ${domain.id}: ${error.response?.data?.errorMessages.join(", ")}`,
        );
      } else {
        throw new Error(
          `Something went wrong while retrieving project ${id} in domain ${domain.id}: ${error.message}`,
        );
      }
    }
  }
  ...
  async getProjectMembers(
    ...
    for (const role of Object.values(project.roles)) {
      try {
        ...
        if (axios.isAxiosError(error)) {
          throw new Error(
            `Something went wrong while retrieving project ${project.key} members: ${error.response?.data?.errorMessages.join(", ")}`,
          );
        } else {
          throw new Error(
            `Something went wrong while retrieving project ${project.key} members: ${error.message}`,
          );
        }
      }
    }
   ...
   */
  async getUserByDomainAndEmail(
    domainId: string,
    email: string,
  ): Promise<JiraShortUser> {
    try {
      ...
      if (axios.isAxiosError(error)) {
        throw new Error(
          `Something went wrong while retrieving user ${email} in domain ${domainId}: ${error.response?.data?.errorMessages.join(", ")}`,
        );
      } else {
        throw new Error(
          `Something went wrong while retrieving user ${email} in domain ${domainId}: ${error.message}`,
        );
      }
    }
  }
  ...
  async getUserByEmail(email: string): Promise<JiraShortUser | null> {
    for (const domain of this.domains) {
      try {
        ...
        if (axios.isAxiosError(error)) {
          console.error(
            `Failed to retrieve user ${email} in domain ${domain.id}: ${error.response?.data?.errorMessages.join(", ")}`,
          );
        } else {
          console.error(
            `Failed to retrieve user ${email} in domain ${domain.id}: ${error.message}`,
          );
        }
      }
    }
