...
<div class="container mx-auto px-4 py-8">
	<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8">
		...
		<button
				class="btn variant-ghost-surface hover:variant-soft-surface"
	...
	<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
		{#each Object.entries(stats) as [key, value], i}
			<div
					class="card variant-glass-surface p-6"
				...
				{#each areas as area, i (area._id)}
					...
					>
						...
						<button
								...
								class="btn w-full {area.active ? 'variant-filled-success hover:variant-soft-success' : 'variant-filled-warning hover:variant-soft-warning'} transition-colors duration-200"
						...
						{#each areas as area, i (area._id)}
							...
							>
								...
								<td class="!p-4">
									<button
											on:click={() => toggleAreaButton(area)}
											class="btn {area.active ? 'variant-filled-success hover:variant-soft-success' : 'variant-filled-warning hover:variant-soft-warning'} transition-all duration-200"
											disabled={toggleLoadingMap.get(area._id)}
									>
										{#if toggleLoadingMap.get(area._id)}
											<div class="loader-sm mr-2" />
...
{#if showDetailsPopup && selectedAreaId}
	<AreaDetailsPopup
            ...
            areas = areas.map(area =>
                area._id === detail.areaId
                    ? { ...area, active: detail.active }
                    : area
            );
        }}
	/>
