<script lang="ts">
	...
	$: filteredActions =
		actionApp?.reactions.filter(
			(action) =>
				action.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
				action.description?.toLowerCase().includes(searchQuery.toLowerCase())
		) || [];

	$: groupedActions = filteredActions.reduce(
		(acc, action) => {
	...
	$: displayedActions =
		selectedCategory === 'All' ? filteredActions : groupedActions[selectedCategory] || [];
...
<div class="space-y-6" in:fade>
		...
		<div class="card variant-ghost-primary p-4">
			<div class="flex items-start gap-4">
				...
				<div>
					...
					<p class="text-sm">
						You'll be able to use {$areaStore.selectedTrigger.variables.length} variables from {$areaStore
							.triggerApp?.name} in your action configuration.
					</p>
				</div>
			</div>
		</div>
	...
	{#if displayedActions.length === 0}
		<div class="flex flex-col items-center justify-center py-12 text-center" in:fade>
			...
			<p class="text-surface-500">Try adjusting your search terms</p>
		</div>
	{:else}
				...
				>
					<svelte:fragment slot="footer">
						{#if action.authorizations?.length}
							<div class="badge variant-soft-warning">Auth Required</div>
						{/if}
						{#if action.variables_supported}
							<div class="badge variant-soft-primary">Supports Variables</div>
						{/if}
					</svelte:fragment>
				</TriggerBtn>
			{/each}
	...
	<div class="card variant-ghost p-4">
		<div class="flex items-start gap-4">
			...
			<div class="text-sm">
				...
				<p>
					Select what should happen when your trigger conditions are met. You'll be able to
					configure the specific details in the next step.
				</p>
				{#if $areaStore.selectedTrigger?.variables?.length}
					<p class="mt-2">
						Look for actions with the "Supports Variables" badge to make the most of your trigger's
						data.
					</p>
				{/if}
			</div>
