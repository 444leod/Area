	...
	<div class="card variant-ghost p-4">
		<div class="flex items-start gap-4">
			...
			<div class="text-sm">
				...
				<p>
					Select the app that will perform actions when your trigger conditions are met. You'll be
					able to configure the specific action in the next step.
				</p>
				{#if $areaStore.selectedTrigger?.variables?.length}
					<p class="mt-2 text-primary-500">
						💡 You'll be able to use {$areaStore.selectedTrigger.variables.length} variables from your
						trigger in the action setup.
					</p>
				{/if}
			</div>
		</div>
