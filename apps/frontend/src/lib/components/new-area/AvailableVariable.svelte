<script lang="ts">
	...
	export let dynamicVariables: Writable<
		{ name: string; type: string; description: string; template: string }[]
	>;

	let isOpen = true; // Always open on desktop
...
<div class="w-full lg:w-1/4 mb-4 lg:mb-0 flex flex-col h-full">
	<div class="card variant-soft p-4 flex flex-col h-full">
		<button
			on:click={() => (isOpen = !isOpen)}
			class="flex items-center justify-between w-full text-left lg:cursor-default mb-4"
		>
		...
		<div class="flex-grow overflow-hidden" class:hidden={!isOpen} transition:slide|local>
			<div class="h-full flex flex-col">
						...
						<li class="border border-surface-300-600-token rounded-lg overflow-hidden">
							...
							{#if expandedVariable === variable.name}
								<div class="p-2 bg-surface-100-800-token">
									...
									<div
										class="mt-2 flex items-center justify-between bg-surface-200-700-token p-2 rounded"
									>
				...
				<p class="mt-4 text-sm font-medium">
					Use variables like this: <code class="bg-surface-200-700-token p-1 rounded"
						>{'{{variableName}}'}</code
					>
				</p>
			</div>
