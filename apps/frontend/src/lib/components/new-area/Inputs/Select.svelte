<script lang="ts">
	...
	$: selectVariables = dynamicVariables.filter(
		(v) =>
			v.type === 'string' ||
			v.type === 'enum' ||
	...
	$: selectedVariableDetails = selectedVariable
		? selectVariables.find((v) => v.name === selectedVariable)
		: null;
</script>
...
{#if isAction}
	...
	<div class="relative rounded-lg variant-ghost p-4" transition:fade>
		<div class="relative">
			<select class="select w-full" {required} value={selectValue} on:change={handleSelectChange}>
		...
		<div class="mt-2" transition:slide>
			...
			{:else}
				<div class="relative">
					...
					<div
						class="absolute right-2 top-1/2 -translate-y-1/2 text-surface-500 pointer-events-none"
					>
						<ListFilter class="w-4 h-4" />
					</div>
				</div>
