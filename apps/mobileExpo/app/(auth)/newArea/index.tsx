  ...
  const renderStepContent = () => {
    switch (currentStep) {
      ...
      case 2:
        return (
          <>
            <Title style={styles.stepTitle}>Choose {currentStep === 0 ? 'a Trigger' : 'an Action'} App</Title>
            {apps
              ...
              .map(app => (
                <Card key={app.id} style={styles.appCard} onPress={() => selectApp(app, currentStep === 0 ? 'trigger' : 'action')}>
                  <Card.Title title={app.name} left={() => <MaterialCommunityIcons name="puzzle" size={24} color={theme.colors.primary} />} />
                </Card>
              ))}
          </>
        );
      ...
      case 3:
        ...
        return (
          <>
            <Title style={styles.stepTitle}>Select {currentStep === 1 ? 'Trigger' : 'Action'}</Title>
            {items.map(item => (
              <Button key={item} mode="outlined" style={styles.itemButton} onPress={() => selectTriggerOrAction(item, currentStep === 1 ? 'trigger' : 'action')}>
                {item}
              </Button>
            ))}
          </>
      ...
      case 4:
        return (
          <>
            ...
            <TextInput label="Automation Name" value={automationName} onChangeText={setAutomationName} style={styles.input} mode="outlined" />
            ...
            {selectedAction === 'SEND_EMAIL' && (
              <>
                ...
                />
                <TextInput label="Subject" value={reactionDetails.subject} onChangeText={text => setReactionDetails({ ...reactionDetails, subject: text })} style={styles.input} mode="outlined" />
              </>
            )}
          </>
        );
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <Card style={styles.card}>
        <Card.Title title={`Step ${currentStep + 1} of ${steps.length}`} subtitle={steps[currentStep]} />
